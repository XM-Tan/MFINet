...
from visual_utils import ImageFilelist, compute_per_class_acc, compute_per_class_acc_gzsl, prepare_semantic_label, add_glasso, add_dim_glasso
...
def test_zsl(opt, model_baseline, testloader, semantic, test_classes, classindex2name):
    GT_targets = []
    predicted_labels = []
    image_embedding_class_out = torch.empty(1, 768).cuda()
    res18_diet_features_out = torch.empty(1, 1000).cuda()
    with torch.no_grad():
        for i, (input, target, impath, matrix, img_loc) in tqdm(enumerate(testloader),total = len(testloader)):
            ...
            for index_impath in impath:
                    ...
                    visual_label_unseen.append(visual_label1)
            if opt.cuda:
                input = input.cuda()
                target = target.cuda()
            output_class, output_attr, mask_feature, mask_label,loss_mask,image_embedding_class,_, res18_diet_features = model_baseline(input, semantic, visual_label_unseen, is_mask = False)
            image_embedding_class_out = torch.cat((image_embedding_class_out, image_embedding_class),dim=0)
            res18_diet_features_out = torch.cat((res18_diet_features_out, res18_diet_features),dim=0)
    ...
    acc_all, acc_avg = compute_per_class_acc(visual_utils_my_all.map_label(torch.from_numpy(GT_targets), test_classes).numpy(),
                                     np.array(predicted_labels), test_classes.numpy())

    if opt.all:
        return acc_all * 100, image_embedding_class_out, res18_diet_features_out
    else:
        return acc_avg * 100, image_embedding_class_out, res18_diet_features_out
...
def test_gzsl(opt, model_baseline, testloader, semantic, test_classes, classindex2name, calibrated_stacking_number):
    GT_targets = []
    predicted_labels = []
    with torch.no_grad():
        for i, (input, target, impath, matrix, img_loc) in tqdm(enumerate(testloader), total = len(testloader)):
            ...
            if opt.cuda:
                input = input.cuda()
                target = target.cuda()
            output, _, _, pre_class,_, image_embedding_class,_,res18_diet_features= model_baseline(input, semantic, visual_label_unseen, is_mask=False)
    ...
    acc_all, acc_avg = compute_per_class_acc_gzsl(GT_targets,
                                     np.array(predicted_labels), test_classes.numpy())
    if opt.all:
        return acc_all * 100 , image_embedding_class, res18_diet_features
    else:
        return acc_avg * 100 , image_embedding_class, res18_diet_features
...
def get_loader(opt, data):
    ...
    if opt.train_mode == 'distributed':
        ...
        trainloader = DataLoader(dataset=dataset_train, batch_sampler=sampler, num_workers=4, pin_memory=True)
                # exit()
    ...
    if opt.gzsl == 'gzsl':       
        ...
        return trainloader, testloader_unseen, visloader
...
def Loss_fn(opt, loss_log, reg_weight, criterion, criterion_regre, model, output, pre_semantic, label_a, label_v, embedding_for_sc):
    # for Layer_Regression:  # 对于回归层
    loss = 0
    ...
    if opt.class_embedding == "w2v":
        if reg_weight['final']['w2v'] > 0:
            loss_semantic = reg_weight['final']['w2v'] * criterion_regre(pre_semantic, label_a)
            loss_log['l_semantic_final'] += loss_semantic.item()
            loss += loss_semantic
    elif opt.class_embedding == "bert":
        if reg_weight['final']['bert'] > 0:
            loss_semantic = reg_weight['final']['bert'] * criterion_regre(pre_semantic, label_a)
            loss_log['l_semantic_final'] += loss_semantic.item()
            loss += loss_semantic
