...
def test_zsl(opt, model_baseline, testloader, semantic, test_classes, classindex2name):
    GT_targets = []
    predicted_labels = []
    predictclass_result = torch.tensor([]).cuda()  
    with torch.no_grad():
        for i, (input, target, impath, matrix, img_loc) in tqdm(enumerate(testloader),total = len(testloader)):
            ...
            if opt.cuda:
                input = input.cuda()
                target = target.cuda()
            output_class, output_attr, mask_feature, mask_label,loss_mask,_,_ = model_baseline(input, semantic, visual_label_unseen, is_mask = False)
            # print("output_class:", output_class.shape)  # torch.Size([6, 10])
            # exit()
            predictclass_result = torch.cat((predictclass_result, output_class), dim=0)  # 将outpredict_class拼接到predictclass_result上
    ...
    if opt.all:
        return acc_all * 100, predictclass_result
    else:
        return acc_avg * 100, predictclass_result
    ...
    with torch.no_grad():
        for i, (input, target, impath, matrix, img_loc) in tqdm(enumerate(testloader), total = len(testloader)):
            ...
            if opt.cuda:
                input = input.cuda()
                target = target.cuda()
            output, _, _, pre_class,_, _,_= model_baseline(input, semantic, visual_label_unseen, is_mask=False)
            predictclass_result = torch.cat((predictclass_result, output_class), dim=0)  # 
